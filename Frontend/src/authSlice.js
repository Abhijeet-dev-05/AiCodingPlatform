...
export const checkAuth = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            const errorMessage = error.response?.data?.message || error.response?.data || error.message || 'Authentication check failed';
            return rejectWithValue(errorMessage);
        }
    }
);
...
export const googleAuthUser = createAsyncThunk(
    ...
    async (googleToken, { rejectWithValue }) => {
        try {
            const response = await axiosClient.post('/user/google-auth', { googleToken });
            return response.data.user;
        } catch (error) {
            const errorMessage = error.response?.data?.message || error.response?.data || error.message || 'Google authentication failed';
            return rejectWithValue(errorMessage);
        }
    }
);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(checkAuth.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload === null ? null : (typeof action.payload === 'string' ? action.payload : (action.payload?.message || 'Something went wrong'));
                state.isAuthenticated = false;
                state.user = null;
            })
