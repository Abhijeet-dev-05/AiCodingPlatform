...
const { initializeCard } = require("../utils/fsrsAlgorithm");
...
const submitCode = async (req, res) => {
    ...
    try {
        ...
        if (status === 'accepted' && !req.user.problemSolved.includes(problemId)) {
            ...
            try {
                const existingCard = await SpacedRepetition.findOne({ userId, problemId });
                if (!existingCard) {
                ...
                // Don't fail the submission if SR card creation fails
                console.error("Spaced repetition auto-add error:", srErr.message);
            }
        }
